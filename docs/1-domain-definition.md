...
2. [핵심 도메인 모델](#2-핵심-도메인-모델)
   ...
   - 2.3 [TodoShare (할일 공유)](#23-todoshare-할일-공유)
   - 2.4 [Notification (알림)](#24-notification-알림)
   - 2.5 [용어 정의](#25-용어-정의)
3. [주요 기능](#3-주요-기능)
   - 3.1 [인증 및 사용자 관리](#31-인증-및-사용자-관리)
   - 3.2 [할일 관리 (CRUD)](#32-할일-관리-crud)
   - 3.3 [기한 관리 및 알림](#33-기한-관리-및-알림)
   - 3.4 [캘린더 뷰](#34-캘린더-뷰)
   - 3.5 [공유 기능](#35-공유-기능)
...
**Jan TodoList**는 사용자 인증 기반의 개인 및 협업 할일 관리 애플리케이션입니다. 사용자는 자신의 할일을 체계적으로 관리하고, 기한을 시각적으로 파악하며, 다른 사용자와 공유할 수 있습니다.

### 핵심 가치
- 개인화된 할일 관리
- 직관적인 기한 관리 (리스트 및 캘린더 뷰)
- 협업 및 공유 기능
- 외부 캘린더 서비스 연동
...
### 2.3 TodoShare (할일 공유)
- 사용자 간 할일 공유 관계
- 속성: 공유자, 피공유자, 권한(읽기/쓰기)

### 2.4 Notification (알림)
- 기한 임박 알림
- 속성: 대상 할일, 알림 시각, 읽음 여부

### 2.5 용어 정의

#### 아카이브 (Archive)
완료된 할일을 별도 저장 공간에 보관하는 기능. 리스트 뷰에는 기본적으로 표시되지 않으나 "완료된 할일 보기" 옵션을 통해 조회 가능. 검색 기능으로도 접근 가능.

#### 권한 (Permission)
할일 공유 시 부여되는 접근 권한:
- **읽기 권한 (Read)**: 할일의 제목, 설명, 기한, 상태를 조회만 가능. 수정 불가.
- **쓰기 권한 (Write)**: 할일의 모든 내용(제목, 설명, 기한, 상태)을 조회 및 수정 가능. 단, 삭제는 불가능 (소유자만 삭제 가능).

#### 우선순위 (Priority)
할일의 중요도를 나타내는 속성:
- **높음 (High)**: 긴급하고 중요한 할일. 빨간색으로 표시. 같은 기한 내에서 최우선 정렬.
- **보통 (Medium)**: 일반적인 할일. 파란색으로 표시. 기본값.
- **낮음 (Low)**: 급하지 않은 할일. 회색으로 표시. 같은 기한 내에서 후순위 정렬.

#### 알림 (Notification)
사용자에게 할일 기한을 상기시키는 기능:
- **앱 내 알림**: 웹 애플리케이션 내 알림 센터에 표시
- **브라우저 푸시 알림**: 브라우저 알림 권한 허용 시 실시간 푸시 알림
- **이메일 알림 (선택)**: 사용자 설정에서 활성화 시 이메일로도 알림 발송

알림은 사용자 설정에서 켜거나 끌 수 있으며, 알림 시간(24시간 전, 1시간 전)도 커스터마이징 가능.

#### 기한 (Due Date)
할일을 완료해야 하는 목표 날짜 및 시간. 선택사항이며, 설정하지 않을 경우 "기한 없음"으로 분류. 기한이 설정된 할일은 기한 가까운 순으로 자동 정렬.

#### 공유 (Share)
할일의 소유자가 다른 사용자에게 해당 할일에 대한 접근 권한을 부여하는 기능. 공유된 할일은 공유받은 사용자의 할일 목록에도 표시되며, 부여된 권한에 따라 조회 또는 수정 가능.
...
### 3.3 기한 관리 및 알림
- **기한 임박 강조**: 기한이 가까운 할일 상단 노출
- **알림 기능**: 설정된 기한 도래 시 알림
- **시각적 표시**: 기한 상태에 따른 색상 구분

### 3.4 캘린더 뷰
- **월간 캘린더**: 할일 기한을 캘린더에 표시
- **일별 할일 조회**: 특정 날짜의 할일 목록
- **외부 캘린더 연동**: 네이버, 구글 캘린더 동기화

### 3.5 공유 기능
- **할일 공유**: 특정 사용자에게 할일 공유
- **권한 관리**: 읽기 전용 또는 수정 가능
- **공유 해제**: 소유자가 공유 취소 가능
...
3. 기한 1일 전 알림 수신
4. 캘린더 뷰에서 이번 주 할일 한눈에 확인

### 시나리오 3: 협업
1. 사용자 A가 할일 생성 (예: "팀 회의 준비")
2. 사용자 B에게 해당 할일 공유 (쓰기 권한)
3. 사용자 B가 할일 내용 수정 및 완료 처리
4. 두 사용자 모두 변경사항 확인 가능

### 시나리오 4: 외부 캘린더 연동
1. 설정에서 구글 캘린더 연동 활성화
2. Jan TodoList의 할일이 구글 캘린더에 자동 동기화
3. 구글 캘린더에서 할일 기한 확인
4. 양방향 동기화 (선택사항)
...
### 6.4 외부 연동
- OAuth 2.0 기반 외부 캘린더 인증
- API 통신: REST 또는 GraphQL

### 6.5 UI/UX
- 반응형 웹 디자인
- 리스트 뷰 / 캘린더 뷰 탭 전환
- 드래그 앤 드롭으로 할일 순서 변경 (선택)
...
#### 기한 관리 및 알림 [REQ-004, REQ-005]
- **알림 전송 성공률**: 99% 이상
- **알림 정시 전송률**: 설정 시간 ±5분 이내 95% 이상
- **기한 임박 할일 정렬 정확도**: 100%

**인수 기준**:
- [ ] 기한이 24시간 이내인 할일은 목록 상단에 표시
- [ ] 기한 24시간 전 알림 발송
- [ ] 기한 1시간 전 알림 발송 (사용자 설정 시)
- [ ] 완료된 할일은 알림 발송 안 함
- [ ] 알림 읽음 표시 기능 동작
...
- **OAuth 인증 성공률**: 95% 이상
- **동기화 성공률**: 95% 이상
- **동기화 주기**: 15분마다 또는 수동 동기화

**인수 기준**:
- [ ] 구글 캘린더 연동 인증 가능
- [ ] 네이버 캘린더 연동 인증 가능
- [ ] Jan TodoList → 외부 캘린더 동기화 (단방향)
- [ ] 동기화 실패 시 사용자에게 알림
- [ ] 연동 해제 기능 동작
...
**인수 기준**:
- [ ] 이메일로 다른 사용자에게 할일 공유 가능
- [ ] 읽기 권한 사용자는 조회만 가능
- [ ] 쓰기 권한 사용자는 수정 가능하나 삭제 불가
- [ ] 소유자만 할일 삭제 가능
- [ ] 공유 해제 기능 동작
- [ ] 공유받은 할일 목록에서 확인 가능
...
- **공유 기능 사용률**: 전체 사용자 중 20% 이상
- **외부 캘린더 연동률**: 전체 사용자 중 30% 이상
- **평균 세션 시간**: 5분 이상
- **사용자 만족도 (NPS)**: 50 이상
...
| REQ-005 | 기한 임박 알림 기능 | 3.3, 5.2 | 필수 | 설계 완료 | 24시간/1시간 전 알림 |
| REQ-006 | 캘린더 형식으로 할일 기한 표시 | 3.4 캘린더 뷰 | 필수 | 설계 완료 | 월간 뷰, 일별 조회 |
| REQ-007 | 외부 캘린더 연동 (네이버/구글) | 3.4, 6.4 | 높음 | 설계 완료 | OAuth 2.0 인증 |
| REQ-008 | 다른 사용자와 공유 가능 | 3.5, 5.3 | 높음 | 설계 완료 | 권한 관리 포함 |
| REQ-009 | 할일 리스트 조회 | 3.2 | 필수 | 설계 완료 | 정렬, 필터링 포함 |
| REQ-010 | 완료된 할일 관리 (아카이브) | 5.1, 2.5 | 중간 | 설계 완료 | 별도 뷰 제공 |
