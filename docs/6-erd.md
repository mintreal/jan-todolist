...
| `due_date` | DATE | NULL | NULL | 기한 (날짜만, 시간 제외) |
...
- 기한은 선택 사항 (NULL 허용)
- 기한은 날짜만 저장 (시간은 00:00:00으로 통일)
- 과거 날짜 기한은 허용하지 않음 (애플리케이션 레벨 검증)
- `is_completed` 기본값은 FALSE (미완료)
- `updated_at`은 수정 시 자동 업데이트 (트리거 또는 애플리케이션 레벨)

**정렬 기준** (애플리케이션 레벨):
```sql
ORDER BY
  CASE WHEN due_date IS NULL THEN 1 ELSE 0 END,  -- 기한 없음은 마지막
  due_date ASC,                                   -- 기한 가까운 순
  created_at DESC                                 -- 동일 기한 시 최신 순
```
...
| idx_todos_due_date | todos | due_date | B-tree | 기한별 정렬 최적화 |
...
-- due_date 인덱스 (기한 기반 정렬 성능 향상)
CREATE INDEX idx_todos_due_date ON todos(due_date);
```

**성능 효과**:
- `idx_todos_user_id`: `WHERE user_id = ?` 쿼리 최적화
- `idx_todos_due_date`: `ORDER BY due_date` 쿼리 최적화
...
```sql
-- 사용자별 기한 정렬 복합 인덱스 (Phase 2)
CREATE INDEX idx_todos_user_due
ON todos(user_id, due_date NULLS LAST, created_at DESC);
...
-- 2. 할일 생성
INSERT INTO todos (user_id, title, is_completed, due_date) VALUES
  -- 홍길동의 할일
  (1, '프로젝트 보고서 작성', false, '2025-11-27'),
  (1, '병원 예약', false, '2025-11-28'),
  (1, '장보기', true, '2025-11-26'),
  (1, '책 읽기', false, NULL),

  -- 김철수의 할일
  (2, '운동하기', false, '2025-11-26'),
  (2, '영어 공부', false, NULL);
```
...
SELECT
  ...
  CASE
    ...
    WHEN t.due_date = CURRENT_DATE THEN '오늘'
    WHEN t.due_date = CURRENT_DATE + 1 THEN '내일'
...
사용자 | 할일              | 완료여부 | 기한       | 기한표시
-------|------------------|---------|-----------|----------
홍길동 | 장보기            | true    | 2025-11-26 | 오늘
홍길동 | 프로젝트 보고서 작성 | false   | 2025-11-27 | 내일
홍길동 | 병원 예약          | false   | 2025-11-28 | 2025-11-28
홍길동 | 책 읽기           | false   | NULL      | 기한 없음
```
